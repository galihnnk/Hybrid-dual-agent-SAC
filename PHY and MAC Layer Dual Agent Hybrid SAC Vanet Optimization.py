...
class TrainingConfigOptimum:
    ...
    max_neighbors: int = 40     # Increased from 10 (better density handling)
...
class AggressiveTrainingConfig:
    ...
    max_neighbors: int = 50         # Handle high density
...
# Global configurations - REALISTIC for vehicular density standards (60-480 vehicles/km)
training_config = AggressiveTrainingConfig()

# REALISTIC PARAMETERS based on vehicular standards
training_config.max_neighbors = 40  # Support up to 480 vehicles/km (realistic gridlock max)
training_config.initial_exploration_factor = 2.0  # Reasonable initial exploration
training_config.min_exploration = 0.3  # Reasonable minimum
training_config.exploration_decay = 0.9997  # Balanced decay
    ...
    def get_config_for_density(vehicles_per_km: int):
        ...
        if vehicles_per_km <= 120:  # Light traffic (30-60 vehicles/km/direction)
            ...
            return {
                ...
                'sinr_target_base': 16.0,  # Lower SINR due to C-NLOS-ENH
                'hidden_units': 512,
                'train_interval': 8,
...
def get_adaptive_targets(neighbor_count: int) -> Dict[str, float]:
    """CHANNEL-AWARE: Adaptive targets based on realistic vehicular density standards"""
    
    # Estimate vehicles per km from neighbor count
    estimated_vehicles_per_km = neighbor_count * 12  # Realistic estimation factor
    
    # Get channel-aware configuration
    ...
    if 'AWGN' in config['channel_model'] or 'R-LOS' in config['channel_model']:
        # Good channel conditions - can be more aggressive
        cbr_adaptation = 0.2
        sinr_adaptation = 3.0
    elif 'H-LOS' in config['channel_model']:
        # Moderate channel conditions
        cbr_adaptation = 0.3
        sinr_adaptation = 4.0
    elif 'H-NLOS' in config['channel_model']:
        # Poor channel conditions
        cbr_adaptation = 0.4
        sinr_adaptation = 6.0
    else:  # C-NLOS-ENH
        # Very poor channel conditions - be conservative
        cbr_adaptation = 0.6
        sinr_adaptation = 8.0
    
    return {
        'CBR_TARGET': config['cbr_target_base'] * (1 - cbr_adaptation * density_factor),
        'SINR_TARGET': max(8.0, config['sinr_target_base'] - sinr_adaptation * density_factor),
        'BEACON_TARGET': 15.0 * (1 - 0.5 * density_factor),  # Reduce beacon rate in dense traffic
        'MCS_TARGET': 7.0 * (1 - 0.4 * density_factor),      # Lower MCS for reliability
        'POWER_TARGET': 25.0 * (1 - 0.3 * density_factor),   # Reduce power in dense traffic
        'CHANNEL_MODEL': config['channel_model'],
        'ESTIMATED_DENSITY': estimated_vehicles_per_km
    }
...
class CentralizedTrainingManager:
    ...
    def add_experience(self, state: List[float], actions: Dict[str, Any], reward: float, next_state: List[float]):
        ...
        try:
            ...
            # ADAPTIVE: Dynamic training interval based on fleet density
            current_avg_neighbors = np.mean([exp[0][2] for exp in self.mac_experiences[-10:]] if len(self.mac_experiences) >= 10 else [5])
            
            if current_avg_neighbors > 15:  # High density
                self.train_interval = 8  # Train less frequently for stability
            elif current_avg_neighbors < 5:  # Low density
                self.train_interval = 3  # Train more frequently for exploration
            else:
                self.train_interval = 5  # Standard interval
...
class MACAgent(UltraExplorativeSACAgent):
    ...
    def get_density_aware_weights(self, cbr: float, sinr: float, neighbor_count: int) -> Dict[str, float]:
        """MAC-specific density-aware weight adjustment with CHANNEL AWARENESS"""
        base_weights = get_reward_weights(cbr, sinr)
        
        # Get channel-aware targets
        targets = get_adaptive_targets(neighbor_count)
        estimated_density = targets['ESTIMATED_DENSITY']
        channel_model = targets['CHANNEL_MODEL']
        
        # Channel-specific weight adjustments
        if estimated_density <= 120:  # Light traffic (AWGN/R-LOS)
            base_weights['W1'] *= 0.9       # Relax CBR focus
            base_weights['W_BEACON'] = 0.6  # Encourage beaconing for connectivity
            base_weights['W_MCS'] = 1.8     # Can use higher MCS
            base_weights['W5'] *= 0.4       # Minimal cooperation needed
            
        elif estimated_density <= 240:  # Moderate traffic (H-LOS/H-NLOS)
            base_weights['W1'] *= 1.1       # Standard CBR focus
            base_weights['W_BEACON'] = 1.0  # Balanced beacon control
            base_weights['W_MCS'] = 1.4     # Moderate MCS preference
            base_weights['W5'] *= 0.7       # Some cooperation
            
        elif estimated_density <= 360:  # Heavy traffic (C-NLOS-ENH)
            base_weights['W1'] *= 1.4       # Strong CBR focus
            base_weights['W_BEACON'] = 2.0  # Strong beacon penalty
            base_weights['W_MCS'] = 2.2     # Prefer robust MCS
            base_weights['W5'] *= 1.3       # Good cooperation
            
        else:  # Gridlock (C-NLOS-ENH Extreme) - MAX 480 vehicles/km
            base_weights['W1'] *= 1.8       # VERY strong CBR focus
            base_weights['W_BEACON'] = 3.0  # VERY strong beacon penalty
            base_weights['W_MCS'] = 3.0     # VERY robust MCS only
            base_weights['W5'] *= 1.8       # Strong cooperation
...
class PHYAgent(UltraExplorativeSACAgent):
    ...
    def get_density_aware_weights(self, cbr: float, sinr: float, neighbor_count: int) -> Dict[str, float]:
        """PHY-specific density-aware weight adjustment with CHANNEL AWARENESS"""
        base_weights = get_reward_weights(cbr, sinr)
        ...
        # Channel-specific weight adjustments
        if estimated_density <= 120:  # Light traffic (AWGN/R-LOS)
            base_weights['W3'] *= 1.3       # Focus on high SINR quality
            base_weights['W2'] *= 0.6       # Allow higher power for coverage
            base_weights['W_INTERFERENCE'] = 0.4  # Minimal interference concern
            base_weights['W_COVERAGE'] = 1.2      # Reward coverage
            base_weights['W5'] *= 0.3       # Minimal cooperation
            
        elif estimated_density <= 240:  # Moderate traffic (H-LOS/H-NLOS)
            base_weights['W3'] *= 1.1       # Moderate SINR focus
            base_weights['W2'] *= 0.9       # Slight power penalty
            base_weights['W_INTERFERENCE'] = 1.0  # Moderate interference awareness
            base_weights['W_COVERAGE'] = 0.8      # Some coverage bonus
            base_weights['W5'] *= 0.6       # Some cooperation
            
        elif estimated_density <= 360:  # Heavy traffic (C-NLOS-ENH)
            base_weights['W3'] *= 0.9       # Relax SINR (channel is poor anyway)
            base_weights['W2'] *= 1.5       # Strong power penalty
            base_weights['W_INTERFERENCE'] = 2.5  # Strong interference penalty
            base_weights['W_COVERAGE'] = 0.3      # Coverage less important
            base_weights['W5'] *= 1.4       # Good cooperation
            
        else:  # Gridlock (C-NLOS-ENH Extreme)
            base_weights['W3'] *= 0.8       # Very tolerant SINR (survival mode)
            base_weights['W2'] *= 2.5       # EXTREME power penalty
            base_weights['W_INTERFERENCE'] = 4.0  # EXTREME interference penalty
            base_weights['W_COVERAGE'] = 0.1      # Coverage unimportant
            base_weights['W5'] *= 2.0       # EXTREME cooperation
    ...
    def _process_batch_fixed(self, batch_data: Dict[str, Any], addr: Tuple[str, int]) -> Dict[str, Any]:
        ...
        for vehicle_id, vehicle_data in batch_data.items():
            try:
                ...
                # ADAPTIVE: Estimate vehicle density and update configuration
                neighbor_count = int(state[2])
                estimated_vehicles_per_km = neighbor_count * 8  # Rough estimation
                
                # Update configuration for extreme densities
                if estimated_vehicles_per_km > 600 or estimated_vehicles_per_km < 120:
                    if vehicle_id in self.vehicle_nodes:
                        self.vehicle_nodes[vehicle_id].update_density_configuration(estimated_vehicles_per_km)
